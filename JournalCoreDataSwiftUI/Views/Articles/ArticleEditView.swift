            ...
            VStack {
                ...
                Button(action: {
                    if !self.nom.isEmpty && !self.description.isEmpty {
                        self.articleController.updateArticle(article: self.article, nom: self.nom, desc: self.description, qte: Int16(self.qte)!, idDepot: selectedDepot!.idDepot!, idCategorie: (selectedCategorie!.idCategorie)!)
